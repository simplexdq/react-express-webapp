...
router.post(
  ...
  async (req, res) => {
    ...
    //Build social object
    profileFields.social = {} //without this line, the next if... lines will have errors, because it can not get social items
...
router.get('/user/:user_id', async (req, res) => {
  try {
    ...
    if (!profiles) return res.status(400).json({ mas: 'Profile not found' })

    res.json(profiles)
  } catch (err) {
    console.error(err.message)
    if (err.kind == 'ObjectId') {
      return res.status(400).json({ mas: 'Profile not found' })
    }
    res.status(500).send('Server Error')
  }
})
